	...
	: wxWindow(parent, wxID_ANY, wxDefaultPosition, size)
{
	...
	searchBar = new TextBox(this, wxEmptyString, wxSize(size.GetWidth() - 60, size.GetHeight()), wxTE_PROCESS_ENTER, wxColor(35, 65, 75), wxColor(30, 55, 64), wxColor(177, 231, 250), textFont);
	wxFont searchFont(wxFontInfo(13).FaceName("Dosis").Bold());
	searchBar->SetFont(searchFont);

	readOnlySearchBar = new TextBox(this, wxEmptyString, wxSize(size.GetWidth() - 60, size.GetHeight()), wxTE_READONLY, wxColor(35, 65, 75), wxColor(30, 55, 64), wxColor(177, 231, 250), textFont);
	...
	mainSizer->Add(readOnlySearchBar, 1, wxEXPAND, 0);
	mainSizer->Add(searchSVG, 0, wxEXPAND, 0);
	mainSizer->Hide(searchBar);
	SetSizer(mainSizer);
}
...
void SearchBar::reset()
{
	...
	{
		mainSizer->Hide(searchBar);
		mainSizer->Show(readOnlySearchBar, true);
	}
	else
	{
		mainSizer->Show(searchBar);
		mainSizer->Hide(readOnlySearchBar, true);
	}
	Layout();
	Refresh();
