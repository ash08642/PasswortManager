	...
	:wxWindow(parent, id, pos, size)
{
    ...
    confirmPasswordBox = new TextBox(this, wxEmptyString, wxSize(264, 34), wxTE_PASSWORD | wxTE_PROCESS_ENTER, *appSetting->order6, *appSetting->order61, *appSetting->order1, *appSetting->order3, textFont);
    confirmOpenEyeSVG = new SVGPanel(this, wxSize(34, 34), closeEyeString, *appSetting->order6, *appSetting->order61);
    ...
    confirmShownPasswordBox = new TextBox(this, wxEmptyString, wxSize(264, 34), wxTE_PROCESS_ENTER, *appSetting->order6, *appSetting->order61, *appSetting->order1, *appSetting->order3, textFont);
    confirmCloseEyeSVG = new SVGPanel(this, wxSize(34, 34), openEyeString, *appSetting->order6, *appSetting->order61);
    ...
    signUpButton = new CustomButton(this, wxSize(100,30), *appSetting->order6, *appSetting->order61, appSetting->selected_language[AppSetting::TextKeys::CreateNewAccount], true, labelFont, *appSetting->order3, false);
    alreadyRegisteredButton = new CustomButton(this, wxDefaultSize, this->GetBackgroundColour(), this->GetBackgroundColour(), appSetting->selected_language[AppSetting::TextKeys::AlreadyHaveAnAccount], false, labelFont, *appSetting->order3, true);
...
bool SignUpPanel::isPasswordStrong()
{
    std::string pass = passwordBox->get_textCtrl()->GetValue().ToStdString();
    bool strong{ false };
    int minPassLen = 8;
    bool hasLower{ false };
    bool hasUpper{ false };
    ...
    if (hasLower && hasUpper && hasDigit && hasSpecial && passLen >= minPassLen) {
        return true;
    }
